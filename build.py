...
(function(){
  ...
  (function(){
    ...
    function settleTo(toC, cb){ tx=toC; onSettle=cb; if(!raf) raf=requestAnimationFrame(tick); }
    ...
    // tap: the blob flows to the tapped tab while the content springs down
    tabs.forEach(function(t, idx){ var href=t.getAttribute&&t.getAttribute("href"); if(!href) return;
      t.addEventListener("click", function(e){ if(didDrag) return; e.preventDefault();
        if(!RM){ startLiquid(slotCenter(ai)); setCand(idx); tx=slotCenter(idx); if(!raf) raf=requestAnimationFrame(tick); }
        navTo(href); }); });

    var down=false, drag=false, didDrag=false, sx=0;
    function onMove(e){
      if(!down) return;
    ...
    function end(){
      ...
      var ci=indexAt(hx), t=tabs[ci], href=t&&t.getAttribute&&t.getAttribute("href");
      if(ci!==ai && href){ tx=slotCenter(ci); if(!raf) raf=requestAnimationFrame(tick); navTo(href); }
      else if(ci!==ai && t && t.id==="settings-btn"){ settleTo(slotCenter(ai), function(){ bar.classList.remove("tb-drag"); clearCand(); placeIdle(); }); bookUp(); t.click(); }
      else { settleTo(slotCenter(ai), function(){ bar.classList.remove("tb-drag"); clearCand(); placeIdle(); }); bookUp(); }
