...
(function(){
  ...
  // ---- tab bar: sliding highlight + spring page transition --------------
  // Every tab navigation (tap OR drag-release) plays the same transition: the
  // current page content springs DOWN, then the next page's content springs UP.
  // Dragging slides a single highlight smoothly; tapping is instant-to-trigger.
  (function(){
    ...
    var down=false, drag=false, didDrag=false, sx=0, w=0, cand=ai;
    function indexAt(x){ var n=tabs.length, i=Math.floor(x/(bar.clientWidth/n)); return Math.max(0, Math.min(n-1, i)); }
    function setCand(i){ cand=i; for(var j=0;j<tabs.length;j++) tabs[j].classList.toggle("cand", j===i && j!==ai); }
    ...
    // the shared transition: spring the content down, flag the next page to
    // spring up (EARLY_JS reads px-enter and runs the px-in keyframe)
    function navTo(href){
      try{ sessionStorage.setItem("px-enter","1"); }catch(e){}
      if(book && !RM){ book.style.transition="transform .22s cubic-bezier(.4,0,.7,1), opacity .22s ease"; book.style.transform="translateY(80px)"; book.style.opacity="0";
        setTimeout(function(){ location.href=href; }, 200); }
      else location.href=href;
    }
    // tap to navigate, with the same spring transition (skip if a drag just ran)
    ...
    function onMove(e){
      ...
      if(!drag){ if(Math.abs(e.clientX-sx)<9) return; drag=true; didDrag=true; bar.classList.add("tb-drag");
        w=Math.max(46, Math.round((parseFloat(getComputedStyle(r).fontSize)||16)*3)); thumb.style.width=w+"px"; }
      if(e.cancelable) e.preventDefault();
      var x=e.clientX - bar.getBoundingClientRect().left;
      thumb.style.left=Math.max(0, Math.min(bar.clientWidth-w, x-w/2))+"px";   // 1:1 with the finger
      setCand(indexAt(x));
      // content follows the drag continuously: distance of the finger from the page's own tab
      var hc=(ai + 0.5) * (bar.clientWidth/tabs.length);
    ...
    function end(){
      ...
      var t=tabs[cand], href=t&&t.getAttribute&&t.getAttribute("href");
      if(cand!==ai && href){ navTo(href); }
      else if(cand!==ai && t && t.id==="settings-btn"){ placeIdle(); bookUp(); t.click(); }
      else { placeIdle(); bookUp(); }
