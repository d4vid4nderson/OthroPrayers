...
# The day's offices come from the parish booklet ("A Little Prayer Book"),
# which is what this community actually prays. Compline is the one Hour the
# booklet has no equivalent for, so it stays in its 1928 American Book of
# Common Prayer form (public domain) -- the historic text Western Rite parishes
# draw on. Individual parishes vary some particulars by jurisdiction and local
# custom -- check with your own priest.
WESTERN_OFFICES = [
    ("western-compline", "Compline", "Night Prayer &mdash; the Church&rsquo;s bedtime office.",
     "mono", "At nightfall", 21, 4),
]

# the hub's daily cycle: the booklet's three offices, then Compline
WESTERN_CYCLE = [
    ("pb-morning", "Morning Prayer", "A short form of morning prayer, for use alone.",
     "cross", "On rising", 4, 11),
    ("pb-noon", "Noon Time Prayers", "A short form for the middle of the day.",
     "gospel", "Midday", 11, 17),
...
def western_page():
    ...
    o.append('<p class="res-intro">The hours of the day, in the Western form &mdash; the short '
             'offices of the parish prayer book, with Compline from the 1928 Book of Common '
             'Prayer.</p>')
    o.append(_cycle([(o_title, f"{slug}.html", blurb, emb, when, h0, h1)
                      for slug, o_title, blurb, emb, when, h0, h1 in WESTERN_CYCLE]))
    if PB_CONTENT:
...
_W_PAGE_FN = {"western-compline": western_compline_page}
